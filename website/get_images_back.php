	...
	if ($filename != "." && $filename != "..") {
		...
		if (($width_sum+$new_width) <= $screen_width) {
			...
			for ($i = 0; $i < $numFiles; $i++) {
				if ($i == 0) {
					$output .= "<img src='".$files[$i][0]."' height=180 width=".$files[$i][1]." style='margin-left: 8px; margin-right: $margin'>";
				}
				else if ($i == ($numFiles -1)) {
					$output .= "<img src='".$files[$i][0]."' height=180 width=".$files[$i][1]." style='margin-left: $margin; margin-right: 8px;'>";
				}
				else {
					$output .= "<img src='".$files[$i][0]."' height=180 width=".$files[$i][1]." style='margin-left: $margin; margin-right: $margin'>";
				}
			}
			unset($files);
